        ...
        static void TraceRoute(string hostNameOrAddress, ITelegramBotClient botClient, Message message)
        {
            ...
            for (int i = 1; i < 30; i++)
            {
                ...
                if (reply.Status == IPStatus.TtlExpired || reply.Status == IPStatus.Success)
                {
                    ...
                    if (host == "")
                        botClient.SendTextMessageAsync(message.Chat.Id, $"{i}. {reply.RoundtripTime}ms {reply.Address}", cancellationToken: cancelTokenSource.Token, replyMarkup: GetCloseButton());
                    else
                        botClient.SendTextMessageAsync(message.Chat.Id, $"{i}. {reply.RoundtripTime}ms {host} [{reply.Address}]", cancellationToken: cancelTokenSource.Token, replyMarkup: GetCloseButton());
                    Thread.Sleep(500);
                }
